...
export async function getWordsJson(
  ...
  let inputTextArea = await page.waitForSelector(inputTextFieldBox, {
    visible: true,
  });
  ...
  await wait(2000).then(async () => {
    await page.evaluate((buttonsPanelSelector) => {
      ...
      if (spanishButton) spanishButton.click();
  ...
  for (let i = 0; i < endWordsBoundaries.length; ++i) {
    ...
    try {
      await page.waitForSelector(translationDefinistionsSelector, {
        visible: true,
        timeout: 1000,
      });
      isBreak = true;
    ...
    await page
      .waitForSelector(seeDictButton, { visible: true, timeout: 5000 })
      .then(async () => {
...
interface WordFullTranslations {
  partOfSpeech: string;
  ...
  return await page.evaluate(async (localInput) => {
    ...
    let endWordsBoundaries = localInput
      ...
      .reduce((accumulator: [number], word: string, index: number) => {
        accumulator.push(
          index > 0 ? word.length + accumulator[index - 1] : word.length
        );
        return accumulator;
      }, []);
...
async function getSentenceTranslation(page: puppeteer.Page): Promise<Object> {
  return await page.evaluate(() => {
    ...
    let retObj: { sentenceEnglishTranslation: string } = {
      sentenceEnglishTranslation: '',
    };
    let translateLineElements = document.getElementsByClassName(
      translateSentenceSelector
    );
    if (translateLineElements.length > 0) {
      retObj[sentenceEnglishTranslation] =
        translateLineElements[0].textContent || '';
    }
    return retObj;
  });
}
